...
// - If array is params
// - If string is id
export default function wait(delay = null, scope = null, callback = null, params = null, id = null){
...
export function cancelWaits(ref){
  ...
  if (typeof ref === 'function'){ // Callback
    gsap.killTweensOf(ref);
  } else if (typeof ref.kill === 'function'){ // Tween object
    ref.kill();
  } else if (typeof ref === 'string'){ // id assumed
    // ref = String(ref);
    if (ref.includes('*')){
      let tws = gsap.globalTimeline.getChildren(true, true, true);
      for (let tw of tws){        
        if (ref === '*' || tw.vars.id && stringMatchesWildcardBlob(tw.vars.id, ref)){          
          tw.kill();
        }
      }
    } else {
...
```
*/
const ALPHANUMS = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890';
