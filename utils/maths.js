...
export function tangentPointsBetweenPointAndCircle(targetPt, circlePos, circleRadius){
  ...
  const ta = { x:circlePos.x + circleRadius * Math.sin(t), y:circlePos.y + circleRadius * -Math.cos(t) };
  t = b + a // Tangent angle B
  const tb = { x:circlePos.x + circleRadius * -Math.sin(t), y:circlePos.y + circleRadius * Math.cos(t) };
  return [ta, tb];
...
//  };
 ...
 export function projectAngleRadsFromPoint(pt, angleRads, dist) {
   ...
   return {x: pt.x + dist * Math.cos(angleRads), y: pt.y + dist * Math.sin(angleRads)};		
 ...
 export function projectDistance(ptA, ptB, dist){
   ...
   return {x: ptA.x - dx*(dist/fullDist), y:ptA.y - dy*(dist/fullDist)};			
 ...
 export function rotatePtAroundPtRad(centerPt, pt, angRads, overwrite = false){
  ...
  return {x:Math.cos(angRads) * (pt.x - centerPt.x) - Math.sin(angRads) * (pt.y-centerPt.y) + centerPt.x, y:Math.sin(angRads) * (pt.x - centerPt.x) + Math.cos(angRads) * (pt.y - centerPt.y) + centerPt.y };
